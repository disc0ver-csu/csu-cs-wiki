...
对于$\sum^*$中的任何字 α，若存在一条从初态结到某一终态结的道路，且这条路上所有弧的标记符连接成的字等于$α$，
则称$α$可为 DFA M 所识别（读出或接受）。

前面没有介绍状态转换图的概念，因为觉得表述并不太明确。DFA 用状态转换图非常好理解。下面是结合转换图的 DFA 的一些示例：
...
??? note "思考一下"
    为什么一个“矩阵”就可以表示了呢，其实，根据定义我们发现，行列了DFA定义中的1、2描述。而矩阵的值对应3描述。4、5需要自行补充。
